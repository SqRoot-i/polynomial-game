...
const els = {
    ...
    roomNameInput:$('roomNameInput'), isPublicRoom:$('isPublicRoom'),
    ...
    lobbyChatSection:$('lobbyChatSection'), lobbyChatBox:$('lobbyChatBox'),
    lobbyChatInput:$('lobbyChatInput'), lobbyChatSend:$('lobbyChatSend'),
    voteModal:$('voteModal'), voteStatus:$('voteStatus'), voteTally:$('voteTally'),
...
els.funcInput.addEventListener('keydown', e => { _resetActivity(); if (e.key === 'Enter') handleDeclare(); });
...
document.querySelectorAll('.online-mode-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (btn.dataset.mode === 'join') _startListeningPublicRooms();
        else Network.stopListeningPublicRooms();
    });
});
...
els.createRoomBtn.addEventListener('click', async () => {
    ...
    const roomName = els.roomNameInput.value.trim() || '新しい部屋';
    const isPublic = els.isPublicRoom.checked;
    const { roomId, password } = await Network.createRoom(name, {}, roomName, isPublic);
    els.roomIdDisplay.textContent   = roomId;
    els.roomPassDisplay.textContent = password;
    els.roomInfo.classList.remove('hidden');
    // Show lobby chat
    els.lobbyChatSection.classList.remove('hidden');
    Network.onChat(msg => appendLobbyChat(msg));
...
function bindVoteModal() {
    els.voteZoneA.addEventListener('click', () => handleVoteChoice('A'));
    els.voteZoneB.addEventListener('click', () => handleVoteChoice('B'));
    els.voteChatSend.addEventListener('click', () => {
        const t = els.voteChatInput.value.trim();
        if (!t) return;
        Network.sendChat(t);
        els.voteChatInput.value = '';
    });
    els.voteChatInput.addEventListener('keydown', e => {
        if (e.key === 'Enter') {
            const t = els.voteChatInput.value.trim();
            if (!t) return;
            Network.sendChat(t);
...
els.lobbyChatInput.addEventListener('keydown', e => { if (e.key === 'Enter') sendLobbyChat(); });
...
async function _startOnlineGame() {
    ...
    if (mode === 'global') {
        ...
        rules.globalConstraint = null;
        rules.playerConstraints = {};
        // Map network player IDs to game IDs for constraints
        room.players.forEach((player, i) => {
            const gameId = i === 0 ? 'human' : `bot${i-1}`;
            const row = document.querySelector(`.per-player-row[data-pid="${gameId}"]`);
            if (row) {
                const sel = row.querySelector('[name^=pc_]:checked');
                rules.playerConstraints[gameId] = sel && sel.value !== 'none' ? sel.value : null;
            }
        });
    } else {
        rules.globalConstraint = null;
    ...
    els.setupScreen.classList.add('hidden');
    els.gameScreen.classList.remove('hidden');
    // Hide lobby chat section in setup (now in game screen chat)
    els.lobbyChatSection.classList.add('hidden');
    ...
    els.chatSection.classList.remove('hidden');
    // Chat callbacks are already registered, just re-register for game chat box
    Network.onChat(msg => appendChat(msg));
    ...
    // ホスト: 自分のアクション処理後にUIを更新するリスナー
    document.addEventListener('polynomial-host-update', _onHostUpdate);
}
...
function _onHostUpdate() {
    ...
    if (st.phase === 'roundEnd') {
        ...
        if (winner) addLog(`${winner.name}: ${st.lastFormula}`, 'log-win');
        showModal();
...
function handleMenu() {
    ...
    document.removeEventListener('polynomial-host-update', _onHostUpdate);
    OnlineSync.reset();
    if (Network.isOnline()) Network.leaveRoom();
    els.gameScreen.classList.add('hidden');
    els.modal.classList.add('hidden');
    els.voteModal.classList.add('hidden');
    if (els.solutionHint) els.solutionHint.classList.add('hidden');
...
function renderPerPlayerConstraints() {
    let players;
    // オンラインモードかつルームがある場合: 参加プレイヤー一覧を使用
    if (_onlineMode && Network.getRoom()?.players?.length > 0) {
        players = Network.getRoom().players.map((p, i) => ({
...
function handleAddCard() {
    ...
    if (Network.isOnline()) {
        ...
        OnlineSync.sendAction({ type: 'initiateVote', payload: {} });
        setMsg('カード追加の投票を開始しました…');
        return;
    }
...
function renderAll() {
    ...
    // Vote modal management (online only)
    if (Network.isOnline() && els.voteModal) {
        if (st.pendingVote) {
            els.voteModal.classList.remove('hidden');
            renderVoteModal(st.pendingVote);
        } else {
            els.voteModal.classList.add('hidden');
        }
    }
...
/* ═══════════════════════════════════════════════
   VOTE MODAL
   ═══════════════════════════════════════════════ */
function renderVoteModal(pendingVote) {
    const st     = Game.getState();
    ...
    if (els.voteStatus) {
        els.voteStatus.textContent = `${voted} / ${total} 人が投票済み（${needed} 票で決定）`;
    }
    if (els.voteTally) {
        els.voteTally.innerHTML =
            `<span class="vote-count-a">Zone A: ${countA} 票</span>
             <span class="vote-count-sep">／</span>
             <span class="vote-count-b">Zone B: ${countB} 票</span>`;
    }

    // Disable if already voted
    const myGameId = Network.getMyGameId() || 'human';
    const myVote   = votes[myGameId];
    if (els.voteZoneA) {
        els.voteZoneA.classList.toggle('vote-chosen', myVote === 'A');
        els.voteZoneA.disabled = myVote !== undefined;
        els.voteZoneA.textContent = myVote === 'A' ? '✓ Zone A に追加' : 'Zone A に追加';
    }
    if (els.voteZoneB) {
        els.voteZoneB.classList.toggle('vote-chosen', myVote === 'B');
        els.voteZoneB.disabled = myVote !== undefined;
        els.voteZoneB.textContent = myVote === 'B' ? '✓ Zone B に追加' : 'Zone B に追加';
    }
}

function handleVoteChoice(choice) {
    OnlineSync.sendAction({ type: 'castVote', payload: { choice } });
    // Optimistic UI: disable buttons
    if (els.voteZoneA) els.voteZoneA.disabled = true;
    if (els.voteZoneB) els.voteZoneB.disabled = true;
}

/* ═══════════════════════════════════════════════
   INACTIVITY / SOLUTION HINT
   ═══════════════════════════════════════════════ */
function _resetActivity() {
    _lastActivityTime  = Date.now();
    _solutionHintShown = false;
    if (els.solutionHint) els.solutionHint.classList.add('hidden');
}
...
function _startInactivityTimer() {
    ...
    _inactivityTimer = setInterval(() => {
        const st = Game.getState();
        if (!st || st.phase !== 'playing' || _solutionHintShown) return;
        if (Date.now() - _lastActivityTime < 30000) return;
        _solutionHintShown = true;
        try {
            const sol = Game.botFindSolution('__noconstraint__');
            if (!els.solutionHint) return;
            if (sol) {
                els.solutionHint.textContent = '💡 現在のカードセットには解が存在します';
                els.solutionHint.className = 'solution-hint has-solution';
            } else {
                els.solutionHint.textContent = '⚠️ 現在のカードセットには解が見つかりません（カードを追加してみてください）';
                els.solutionHint.className = 'solution-hint no-solution';
            }
            els.solutionHint.classList.remove('hidden');
            // タップで閉じる
            els.solutionHint.onclick = () => {
                els.solutionHint.classList.add('hidden');
                _solutionHintShown = false;
            };
        } catch(e) { console.warn('solution check failed', e); }
    }, 1000);
}
...
/* ═══════════════════════════════════════════════
   PUBLIC ROOMS
   ═══════════════════════════════════════════════ */
function _startListeningPublicRooms() {
    if (!els.publicRoomsList) return;
    els.publicRoomsList.innerHTML = '<span class="rooms-loading">読み込み中...</span>';
    Network.listenPublicRooms(rooms => {
        if (!els.publicRoomsList) return;
        if (!rooms || rooms.length === 0) {
            els.publicRoomsList.innerHTML = '<span class="rooms-empty">公開ルームはありません</span>';
            return;
        }
        els.publicRoomsList.innerHTML = rooms.map(r => `
          <div class="public-room-item" data-roomid="${r.roomId}">
            <span class="pub-room-name">${r.roomName || '名前なし'}</span>
            <span class="pub-room-host">ホスト: ${r.hostName || '?'}</span>
            <button class="btn btn-outline btn-sm pub-room-join" data-roomid="${r.roomId}">参加</button>
          </div>`).join('');
        els.publicRoomsList.querySelectorAll('.pub-room-join').forEach(btn => {
            btn.addEventListener('click', () => {
                const rid = btn.dataset.roomid;
                if (els.joinRoomId) els.joinRoomId.value = rid;
                if (els.joinPassword) els.joinPassword.focus();
            });
        });
    });
}
...
function appendChat(msg) {
    ...
    // Also forward to vote chat if vote modal is open
    appendVoteChat(msg);
}
