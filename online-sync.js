    ...
    function _processAction(action) {
        ...
        switch (type) {
            ...
            case 'declare': {
                ...
                Network.broadcastState(Game.getState());
                document.dispatchEvent(new CustomEvent('polynomial-host-update'));
                break;
            }
            ...
            case 'initiateVote': {
                if (st.phase !== 'playing' || st.pendingVote) return;
                Game.startVote(gameId);
                Network.broadcastState(Game.getState());
                document.dispatchEvent(new CustomEvent('polynomial-host-update'));
                break;
            }
            ...
            case 'castVote': {
                if (!st.pendingVote) return;
                const voteResult = Game.castVoteOnCard(gameId, payload.choice);
                if (voteResult.done) {
                    if (!Game.addCardToZone(voteResult.zone)) {
                        // デッキが空
                        Game.checkGameOver();
                    }
                }
