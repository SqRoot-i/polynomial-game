...
const Network = (() => {
    ...
    // ── ルーム作成（ホスト） ───────────────────────────────────
    async function createRoom(hostName, gameConfig, roomName, isPublic) {
        if (!_initFirebase()) return _localCreateRoom(hostName, gameConfig);

        const roomId   = generateRoomId();
        const password = generatePassword();
        const rName    = roomName || '新しい部屋';
        _myId   = 'host_' + Date.now();
        _myName = hostName;
        _isHost = true;
        _room   = { roomId, password, roomName: rName, isPublic: !!isPublic, players: [], config: gameConfig };

        await _db.ref(`/rooms/${roomId}`).set({
            meta: { hostId: _myId, password, createdAt: Date.now(), started: false,
                    roomName: rName, isPublic: !!isPublic },
            players: {
        ...
        // 公開ルームの場合 /publicRooms に登録（切断時削除）
        if (isPublic) {
            await _db.ref(`/publicRooms/${roomId}`).set({ roomId, roomName: rName, hostName, createdAt: Date.now() });
            _db.ref(`/publicRooms/${roomId}`).onDisconnect().remove();
        }
    ...
    // ── 公開ルーム一覧 ────────────────────────────────────────
    let _publicRoomsUnsub = null;
    function listenPublicRooms(cb) {
        if (!_initFirebase()) { cb([]); return; }
        const ref = _db.ref('/publicRooms').orderByChild('createdAt').limitToLast(20);
        const handler = snap => {
            const data = snap.val() || {};
            cb(Object.values(data).reverse());
        };
        ref.on('value', handler);
        _publicRoomsUnsub = () => ref.off('value', handler);
    }
    function stopListeningPublicRooms() {
        if (_publicRoomsUnsub) { _publicRoomsUnsub(); _publicRoomsUnsub = null; }
    }
    ...
    return {
        ...
        listenPublicRooms, stopListeningPublicRooms,
        getRoom:      () => _room,
        isOnline:     () => _room !== null,
        isHost:       () => _isHost,
        getMyId:      () => _myId,
